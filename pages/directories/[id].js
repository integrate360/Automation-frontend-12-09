    ...
    const fetchBlog = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/directory/${id}`
        );
  ...
  return (<>
    ...
    <div className="bg-white py-8">
      <div className="container mx-auto px-16">
        <h2 className="text-3xl font-bold text-black font-poppins mb-4 " style={{ width: "1200px", borderBottom: "2px solid #000"}}>
          {post.title}
        </h2>
        <img
          src={post.logo}
        ...
        <div className="flex flex-wrap border border-solid border-gray-300" style={{ width: "1200px"}}>
          ...
          <div className="w-full sm:w-1/2 lg:w-1/4 mb-4 flex px-4">
            <p className="text-black font-poppins mb-1"><FontAwesomeIcon icon={faCalendarAlt} /></p>
            <p className="text-black font-poppins ml-2">{formatDate(post.establishedDate)}</p>
          </div>
        </div>
      </div>
    </div>
